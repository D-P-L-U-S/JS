<?php
    ...
    if(isset($_SESSION['autorisation']) && $_SESSION['autorisation'] == "oui"){
        ...
        if(isset($_SESSION['aut_modif']) && $_SESSION['aut_modif'] == "oui"){
            $req = $bdd->prepare('SELECT Titre, Contenu FROM journal_secret WHERE ID = ? AND Auteur = ?');
            $req->execute(array(
                $_SESSION['update'], $_SESSION['ID']
            ));
            $res = $req->fetch();
        ?>
            <!DOCTYPE html>
            ...
            <head>
                ...
                <title>Mon journal Secret - Modifier un journal</title>
            ...
            <div class="monmenu">
                ...
                <a class="ecrire" href="ecrire.php">
                        <i class="fas fa-edit" id="ico2"></i><div class="div">Créer un journal</div>
                </a>
                ...
                <div class="login2">
                    <h1>Modifier un journal</h1>
                    <hr class="hr">
                    <form action="auth/config.php" method="post" class="form1">
                        <label for="ecrirejs">Titre du journal :</label><br>
                        <textarea name="titre" id="ecrirejs" cols="30" rows="10" required><?php echo $res['Titre']; ?></textarea><br>
                        <label for="ecrirejs2">Qu'est-ce qui se passe ? Qu'est-ce qui s'est passé ?</label><br>
                        <textarea name="contenu" id="ecrirejs2" cols="30" rows="10" required><?php echo $res['Contenu']; ?></textarea><br>
                        <button type="submit" name="saveupdate">Sauvegarder</button>
                    </form>
                </div>
                </div>
